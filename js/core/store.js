...
export async function createStore(bus) {
    const STORAGE_KEY = 'hospital_desktop_v1';
    ...
    function migrateData(oldData, newData) {
        ...
        Object.keys(oldData).forEach(key => {
            ...
            if (Array.isArray(oldData[key])) {
                if (oldData[key].length > 0) migrated[key] = oldData[key];
    ...
    function saveData(data) {
        try {
            ...
            if (bus) bus.emit('store:saved', { timestamp: Date.now() });
        } catch (error) {
    ...
    const store = {
        ...
        update(collection, id, changes) {
            ...
            const updatedItem = { ...items[index], ...changes, updatedAt: Date.now() };
        ...
        getStats() {
            ...
            Object.keys(data).forEach(collection => {
                if (Array.isArray(data[collection])) stats[collection] = data[collection].length;
            });
            return stats;
        },
        ...
        getUpcomingAppointments(days = 7) {
            ...
            return this.query('appointments', {
                dateTime: value => {
                    const appointmentDate = new Date(value);
                    return appointmentDate >= today && appointmentDate < limit && appointmentDate >= new Date();
                }
            });
        },

        reset() {
            if (confirm('¿Estás seguro de resetear?')) {
                data = deepClone(defaultData);
                saveData(data);
                if (bus) bus.emit('store:reset');
                return true;
            }
            return false;
        },
        ...
        subscribe(collection, callback) {
            ...
            const listener = (event) => {
                if (event.detail.collection === collection) callback(event.detail);
            };
            bus.on('store:changed', listener);
            return () => bus.off('store:changed', listener);
        },

...
export function mount(root, { bus, store, user, role }) {
  const state = {
    ...
    isLoading: false,
    showModal: false,
    currentView: role === 'receptionist' ? 'list' : 'calendar',
  ...
  function render() {
    ...
    root.innerHTML = `
      <div class="module-appointments">
          ...
          <div class="flex justify-between items-center">
            ...
            <div class="search-input-wrapper" style="position: relative; width: 450px;">
              ...
              <input type="text" class="input" id="filter-search" 
                     placeholder="Buscar cita por paciente, médico, área o motivo..." 
        ...
        <!-- Fila de Filtros Secundarios y Contador -->
        <div class="flex justify-between mt-3 items-center mb-4">
          <div class="flex gap-2 items-center">
            ${role !== 'receptionist' ? `
              <div class="view-toggle" style="background: #f1f5f9; padding: 2px; border-radius: 8px;">
                <button class="view-toggle-btn ${state.currentView === 'calendar' ? 'active' : ''}" id="btn-view-calendar" style="padding: 0.35rem 1rem; border-radius: 6px; font-size: 0.8rem;">
                  ${icons.calendar} Calendario
                </button>
                <button class="view-toggle-btn ${state.currentView === 'list' ? 'active' : ''}" id="btn-view-list" style="padding: 0.35rem 1rem; border-radius: 6px; font-size: 0.8rem;">
                  ${icons.clipboard} Lista
                </button>
              </div>
            ` : '<div></div>'}
          </div>
          <div class="text-xs font-bold text-muted" id="appointments-count" style="text-transform: uppercase; background: #f8fafc; padding: 4px 10px; border-radius: 4px; border: 1px solid #e2e8f0;">
            Mostrando 0 citas
          </div>
        </div>

        <!-- Contenedores de Vista -->
        <div id="calendar-view" class="${state.currentView === 'calendar' ? '' : 'hidden'}"></div>
        <div id="list-view" class="${state.currentView === 'list' ? '' : 'hidden'}">
          <div id="appointments-header" class="flex justify-between items-center mb-4">
            <h3 style="margin: 0; font-weight: 700; color: var(--themePrimary);">Citas Programadas</h3>
            <div class="text-sm text-muted" id="appointments-count" style="background: #f1f5f9; padding: 0.25rem 0.75rem; border-radius: 20px;">Cargando...</div>
          </div>

          <div id="empty-state" class="hidden text-center" style="padding: 4rem 2rem; background: white; border-radius: 12px; border: 1px dashed #cbd5e1;">
            <div style="font-size: 2.5rem; margin-bottom: 1rem; opacity: 0.3;">${icons.clipboard}</div>
            <h3 style="font-weight: 700; color: #475569; margin-bottom: 0.5rem;">No hay registros</h3>
          ...
          <div id="appointments-table" class="hidden">
            <div class="table-responsive card" style="border-radius: 10px; overflow: hidden; border: 1px solid var(--border);">
              <table class="f-table">
                <thead>
                  <tr>
                    <th>Paciente</th>
          ...
          <div class="modal-content" style="max-width: 800px;">
            ...
            <div style="margin-bottom: 2rem;">
              <div style="font-size: 0.9rem; font-weight: 700; color: var(--modal-section-olive); margin-bottom: 1rem; border-bottom: 1px solid #eee; padding-bottom: 0.5rem;">
                <span style="opacity: 0.7;">${icons.clipboard}</span> INFORMACIÓN ADICIONAL Y ELEGIBILIDAD
              </div>

              <div class="grid grid-2" style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; margin-bottom: 1rem;">
                <div class="form-group">
                  <label class="form-label" style="font-weight: 700; color: var(--modal-text); font-size: 0.85rem;">ASEGURADORA / COBERTURA</label>
                  <input type="text" class="input" id="form-insurance" placeholder="Nombre del seguro..." style="border-color: var(--neutralTertiary); background: var(--white);">
                </div>
                <div class="form-group">
                  <label class="form-label" style="font-weight: 700; color: var(--modal-text); font-size: 0.85rem;">Nº AUTORIZACIÓN</label>
                  <input type="text" class="input" id="form-auth-code" placeholder="Código de autorización..." style="border-color: var(--neutralTertiary); background: var(--white);">
                </div>
              </div>
  ...
  `;
    ...
    elements = {
      ...
      formInsurance: root.querySelector('#form-insurance'),
      formAuthCode: root.querySelector('#form-auth-code'),
        ...
        <tr>
          ...
          <td data-label="Acciones">
            <div class="flex gap-2">
              ${(role === 'admin' || role === 'receptionist') && (appointment.status === 'scheduled' || appointment.status === 'confirmed') ? `
                <button class="btn-circle" style="background: var(--themePrimary); color: white;" data-action="check-in" data-id="${appointment.id}" title="Registrar Llegada (Check-in)">
                  ${icons.successCheck}
                </button>
                <button class="btn-circle" style="background: var(--themeDark); color: white;" data-action="no-show" data-id="${appointment.id}" title="Marcar Inasistencia">
                  ${icons.warning}
                </button>
              ` : ''}
              ${canEdit ? `
  ...
  function getStatusBadge(status) {
    ...
    switch (status) {
      ...
      case 'arrived': icon = icons.successCheck; break;
      case 'no_show': icon = icons.warning; break;
    }
  ...
  function handleListAction(event) {
    ...
    switch (action) {
      ...
      case 'check-in': checkInAppointment(appointment); break;
      case 'no-show': markNoShow(appointment); break;
    }
  }
  ...
  function populateForm(appointment) {
    ...
    if (elements.formInsurance) elements.formInsurance.value = appointment.insuranceCompany || '';
    if (elements.formAuthCode) elements.formAuthCode.value = appointment.authCode || '';
  ...
  function clearForm() {
    ...
    if (elements.formReason) elements.formReason.value = '';
    if (elements.formNotes) elements.formNotes.value = '';
    if (elements.formInsurance) elements.formInsurance.value = '';
    if (elements.formAuthCode) elements.formAuthCode.value = '';
  ...
  function getFormData() {
    ...
    return {
      ...
      insuranceCompany: elements.formInsurance?.value.trim() || '',
      authCode: elements.formAuthCode?.value.trim() || '',
  ...
  async function checkInAppointment(appointment) {
    ...
    try {
      ...
      Logger.log(store, user, {
        action: Logger.Actions.UPDATE,
        module: Logger.Modules.APPOINTMENTS,
        description: `Check-in realizado para cita ID: ${appointment.id}. Paciente en espera.`,
        details: { appointmentId: appointment.id }
      });
      showNotification('Llegada del paciente registrada correctamente.', 'success');
      loadAppointments();
    } catch (error) {
      console.error('Error en check-in:', error);
      showNotification('Error al realizar el check-in', 'error');
    }
  }

  async function markNoShow(appointment) {
    if (!appointment) return;
    const confirm = await hospitalConfirm('¿Cerrar cita por inasistencia del paciente?', 'warning');
    if (!confirm) return;
    ...
    try {
      ...
      // Devolver stock de insumos si los tenía
      if (appointment.supplyIds && appointment.supplyIds.length > 0) {
        for (const sId of appointment.supplyIds) {
          const supply = store.find('suministros', sId);
          if (supply) {
            await store.update('suministros', sId, { stock: (supply.stock || 0) + 1 });
          }
        }
      }
      ...
      Logger.log(store, user, {
        action: Logger.Actions.UPDATE,
        module: Logger.Modules.APPOINTMENTS,
        description: `Inasistencia registrada para cita ID: ${appointment.id}. Recursos liberados.`,
        details: { appointmentId: appointment.id }
      });
      showNotification('Inasistencia registrada y recursos liberados.', 'info');
      loadAppointments();
    } catch (error) {

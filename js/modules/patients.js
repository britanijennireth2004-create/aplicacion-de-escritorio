...
export function mount(root, { bus, store, user, role }) {
  ...
  async function validateForm() {
    ...
    // Comprobar duplicados por DNI (si es nuevo registro)
    if (isValid && !state.editingId && elements.formDni && elements.formDni.value.trim()) {
      const dni = elements.formDni.value.trim();
      const docType = elements.formDocType?.value || 'V';
      const existing = state.patients.find(p => p.dni === dni && p.docType === docType);
      if (existing) {
        window.hospitalFieldValidation.show(elements.formDni, `Este paciente ya existe (${existing.name})`);
        isValid = false;
      }
    }
